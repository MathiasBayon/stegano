...
pub fn simple_decrypt(vector: &[u8], password: &str) -> Result<String, Error> {
    let iv: [u8; 16] = [0; 16];
...
pub mod tests {
    ...
    fn test_simple_encrypt_decrypt() {
        let encrypted = simple_encrypt("Hello, how is the weather today ?", "Password")
            .unwrap_or_else(|err| {
        ...
        assert_eq!(
            simple_decrypt(&encrypted, "Password").unwrap_or_else(|err| {
                eprintln!("Error in test_simple_encrypt_decrypt: {}", err);
                process::exit(1);
            }),
